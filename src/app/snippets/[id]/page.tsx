...
function SnippetDetailPage() {
  ...
  // Validate and type snippetId
  const snippetId = id && typeof id === "string" ? (id as Id<"snippets">) : undefined;

  // If snippetId is invalid, show an error or loading state
  if (!snippetId) {
    return (
      <div className="min-h-screen bg-[#0a0a0f] flex items-center justify-center">
        <p className="text-white text-lg">Invalid snippet ID</p>
      </div>
    );
  }
  ...
  // Handle case where snippet is null (not found)
  if (snippet === null) {
    return (
      <div className="min-h-screen bg-[#0a0a0f] flex items-center justify-center">
        <p className="text-white text-lg">Snippet not found</p>
      </div>
    );
  }
